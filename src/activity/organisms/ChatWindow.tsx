...
export function ChatWindow({
  ...
  return (
    <div
      data-testid={testId}
      className={clsx(
        'flex flex-col bg-discord-bg-primary border border-discord-border rounded-lg shadow-xl overflow-hidden',
        className
      )}
    >
      {/* Header */}
      <div className="px-4 py-3 border-b border-discord-border flex items-center gap-3 bg-discord-bg-secondary">
        {avatar && (
          <Avatar
            src={avatar.src}
        ...
        <div>
          <div className="text-discord-text-normal font-medium">{title}</div>
          {subtitle && (
            <div className="text-xs text-discord-text-muted">{subtitle}</div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {messages.length === 0 && welcomeMessage && (
          <div className="text-center text-discord-text-muted mt-8">
